...
const Reservation = ({ past = false }: { past?: boolean }) => {
  return (
    <div className="bg-background grid w-full grid-cols-1 items-center gap-2 space-y-2 rounded-[20px] p-6 lg:grid-cols-[1fr_180px]">
      ...
      {!past && (
        <button className="order-3 mt-4 ml-auto block w-full rounded-full border border-red-200 bg-red-100/50 px-0 py-2 text-sm transition-colors duration-200 ease-in-out hover:bg-red-100 lg:order-none lg:mt-0">
          Atšaukti rezervaciją
        </button>
      )}
      <p className="text-sm leading-normal lg:col-span-2">
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:gap-4 md:flex-col md:gap-1 lg:col-span-2 lg:flex-row lg:gap-4">
        <p className="text-sm">
